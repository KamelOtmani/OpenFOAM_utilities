...
function set_number_of_cores(){
    local filepath=$1
    local num_cores=$2
    # Set number of processors in the file
    sed -i "s/^numberOfSubdomains.*;$/numberOfSubdomains ${num_cores};/" $filepath
}

# iterate over all decomposeParDict files and set number of cores
for i in $(find $PWD -name decomposeParDict); do
    set_number_of_cores $i $SLURM_NTASKS
done
